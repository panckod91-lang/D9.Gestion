...
const number = value => new Intl.NumberFormat("es-AR", {maximumFractionDigits:3}).format(Number(value)||0);
...
function accountRows() {
  ...
  state.gestion.movimientos.filter(m=>!isAnnulled(m.estado)).forEach(m=>{
    ...
    const a=map.get(id); a.debe+=Number(m.debe)||0; a.haber+=Number(m.haber)||0; a.movimientos.push(m);
  });
...
function renderHome() {
  ...
  $("#kpiOrders").textContent=orders.length; $("#kpiOperations").textContent=ops.length; $("#kpiOperationsAmount").textContent=money(ops.reduce((s,o)=>s+Number(o.total||0),0));
...
function productOptions(selected="") { return '<option value="">Seleccionar producto…</option>'+state.source.productos.filter(p=>Number(p.lista_1)>0).map(p=>`<option value="${esc(p.id)}" ${String(p.id)===String(selected)?"selected":""}>${esc(p.nombre)} · ${money(p.lista_1)}</option>`).join(""); }

function openOperation(order=null) {
  state.currentOrder=order; state.draftItems=(order?.items||[]).map(i=>({id_producto:i.id_producto||i.id, nombre:i.nombre||i.detalle, cantidad:Number(i.cantidad||i.total)||0, precio:Number(i.precio)||0}));
...
async function saveOperation(event) {
  ...
  try{const data=await apiPost("create_operacion",payload);$("#operationDialog").close();toast(`Comprobante ${data.numero} guardado`);await loadAll();showOperationDetail(data.operacion_id,true);}catch(err){toast(err.message,"error")}finally{btn.disabled=false;btn.textContent="Guardar comprobante";}
}
...
function updateReceiptOperations(){const cid=$("#receiptClient").value;$("#receiptOperation").innerHTML='<option value="">A cuenta, sin comprobante específico</option>'+activeOperations().filter(o=>String(o.cliente_id)===String(cid)&&Number(o.saldo)>.005).map(o=>`<option value="${esc(o.operacion_id)}">${esc(o.tipo)} ${esc(o.numero)} · saldo ${money(o.saldo)}</option>`).join("");}
...
function itemsTable(items){return `<table class="detail-lines"><thead><tr><th>Producto</th><th>Cant.</th><th>Unit.</th><th>Total</th></tr></thead><tbody>${items.map(i=>`<tr><td>${esc(i.nombre||i.producto||i.detalle)}</td><td>${number(i.cantidad||i.total)}</td><td>${money(i.precio||i.precio_unitario)}</td><td>${money(i.subtotal||i.total_item||Number(i.cantidad||i.total)*Number(i.precio||i.precio_unitario))}</td></tr>`).join("")}</tbody></table>`}
...
function printWindow(title, body, format="A5") {const win=window.open("","_blank","noopener,noreferrer");if(!win)return toast("El navegador bloqueó la impresión.","error");win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>@page{size:${format} portrait;margin:8mm}*{box-sizing:border-box}body{font:12px Arial,sans-serif;color:#101d2b;margin:0}.head{display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid #17365c;padding-bottom:8px}.head h1{margin:0;font-size:21px}.head p{margin:2px 0}.doc{text-align:right}.doc strong{font-size:18px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:5px 16px;margin:10px 0;padding:8px;background:#f2f6f8}.meta div{display:flex;justify-content:space-between;gap:8px}table{width:100%;border-collapse:collapse}th,td{padding:5px;border-bottom:1px solid #ccd6dc;text-align:left}th{font-size:10px;text-transform:uppercase}th:nth-child(n+2),td:nth-child(n+2){text-align:right}.totals{margin:10px 0 0 auto;width:48%}.totals div{display:flex;justify-content:space-between;padding:4px}.totals .grand{font-size:16px;font-weight:bold;border-top:2px solid #17365c}.foot{margin-top:14px;border-top:1px solid #ccd6dc;padding-top:7px;font-size:10px;color:#52616e}.signature{display:grid;grid-template-columns:1fr 1fr;gap:30px;margin-top:28px}.signature div{border-top:1px solid #222;text-align:center;padding-top:4px}</style></head><body>${body}<script>setTimeout(()=>window.print(),200)<\/script></body></html>`);win.document.close();}
function printOperation(id){const o=state.gestion.operaciones.find(x=>String(x.operacion_id)===String(id));if(!o)return;const cfg=state.gestion.config,items=operationItems(id),subtotal=items.reduce((s,i)=>s+Number(i.subtotal||Number(i.cantidad)*Number(i.precio_unitario)),0);const body=`<div class="head"><div><h1>${esc(cfg.empresa_nombre||"Distribuidora D9")}</h1><p>Comprobante interno</p></div><div class="doc"><strong>${esc(o.tipo)} ${esc(o.numero)}</strong><p>${formatDate(o.fecha)}</p></div></div><div class="meta"><div><span>Cliente</span><b>${esc(o.cliente)}</b></div><div><span>ID</span><b>${esc(o.cliente_id)}</b></div>${o.origen_pedido_id?`<div><span>Pedido origen</span><b>${esc(o.origen_pedido_id)}</b></div>`:""}<div><span>Operador</span><b>${esc(o.usuario||"")}</b></div></div>${itemsTable(items)}<div class="totals"><div><span>Subtotal</span><b>${money(subtotal)}</b></div>${Number(o.descuento_pct)?`<div><span>Descuento ${number(o.descuento_pct)}%</span><b>-${money(subtotal-Number(o.total))}</b></div>`:""}<div class="grand"><span>Total</span><b>${money(o.total)}</b></div><div><span>Saldo</span><b>${money(o.saldo)}</b></div></div>${o.observaciones?`<p><b>Observaciones:</b> ${esc(o.observaciones)}</p>`:""}<div class="signature"><div>Entregó</div><div>Recibí conforme</div></div><div class="foot">${esc(cfg.leyenda||"Comprobante interno no válido como factura.")}</div>`;printWindow(`${o.tipo} ${o.numero}`,body,cfg.impresion||"A5")}
