...
function operationTime(operation){const match=String(operation.created_at||"").match(/(?:T|\s)(\d{2}:\d{2})(?::\d{2})?/);return match?match[1]:"—"}
...
function applySellerSuggestions(){Object.entries(state.sellerSuggestions).forEach(([clientId,suggestion])=>{if(!state.sellerAssignments[clientId])state.sellerAssignments[clientId]=String(suggestion.vendedor_id)});$("#sellerAssignmentMissing").checked=false;renderSellerAssignments();toast("Sugerencias aplicadas. Revisalas antes de guardar.")}
...
function bindEvents(){
  ...
  $("#btnMore").addEventListener("click",()=>$("#moreDialog").showModal());$("#btnNewProduct").addEventListener("click",()=>openProductEditor());$("#btnBulkPrices").addEventListener("click",openBulkPrices);$("#productForm").addEventListener("submit",saveProduct);$("#bulkPriceForm").addEventListener("submit",applyBulkPrices);$("#btnRefreshBulkPreview").addEventListener("click",calculateBulkPreview);$("#mastersPriceList").addEventListener("change",renderMasters);$("#btnNewClient").addEventListener("click",()=>openClientEditor());$("#clientForm").addEventListener("submit",saveClient);$("#btnDeleteClient").addEventListener("click",deleteClient);$("#btnAssignSellers").addEventListener("click",openSellerAssignment);$("#sellerAssignmentSearch").addEventListener("input",renderSellerAssignments);$("#sellerAssignmentMissing").addEventListener("change",renderSellerAssignments);$("#btnApplySellerSuggestions").addEventListener("click",applySellerSuggestions);$("#btnSaveSellerAssignments").addEventListener("click",saveSellerAssignments);$("#sellerAssignmentList").addEventListener("change",event=>{const select=event.target.closest("[data-seller-assignment]");if(select){state.sellerAssignments[select.dataset.sellerAssignment]=select.value;renderSellerAssignments()}});$$("#clientFiscalDetails input, #clientFiscalDetails select").forEach(input=>input.addEventListener("input",updateClientFiscalStatus));$("#btnNewOffer").addEventListener("click",()=>openOfferEditor());$("#offerForm").addEventListener("submit",saveOffer);$("#offerProduct").addEventListener("input",updateOfferProductInfo);$("#btnDeleteOffer").addEventListener("click",deleteOffer);$("#offersSearch").addEventListener("input",renderOffers);$("#offersStatus").addEventListener("change",renderOffers);
