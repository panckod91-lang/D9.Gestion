...
async function saveProduct(event){
  ...
  try{const result=await apiPost("source_save_product",{producto});toast(result.message||"Producto guardado");$("#productDialog").close();await loadAll();renderMasters()}
...
async function applyBulkPrices(event){
  ...
  try{const result=await apiPost("source_bulk_prices",{cambios:state.bulkPriceChanges});toast(`${result.actualizados||0} precios actualizados`);$("#bulkPriceDialog").close();await loadAll();renderMasters()}
...
function commitQuickProduct(product,quantity){
  ...
  else {const offer=currentOfferForProduct(product.id),normal=productPriceForOperation(product);state.draftItems.push({id_producto:product.id,nombre:product.nombre,cantidad:quantity,precio:offer?numeric(offer.precio_oferta):normal,precio_lista:normal,precio_oferta:offer?numeric(offer.precio_oferta):0,usa_oferta:!!offer,oferta_id:offer?.oferta_id||""})}
