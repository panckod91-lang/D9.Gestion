...
function renderOperations() {
  ...
  $("#operationsList").className="card-list"; $("#operationsList").innerHTML=rows.map(o=>{const opType=String(o.tipo).toUpperCase(),commissionStatus=operationCommissionStatus(o),seller=operationSellerInfo(o),active=!isAnnulled(o.estado);return `
    <article class="data-card"><div><h3>${esc(operationTypeLabel(o.tipo))} ${esc(formatOperationNumber(o.numero||""))}</h3><p>${esc(o.cliente||"")} · ${formatDate(o.fecha)}</p><div class="meta">${o.referencia_numero?`<span class="pill amber">Sobre ${esc(formatOperationNumber(o.referencia_numero))}</span>`:(o.origen_pedido_id?`<span class="pill">Pedido ${esc(o.origen_pedido_id)}</span>`:'<span class="pill">Carga manual</span>')}<span class="pill ${commissionStatus==="PENDIENTE"?"amber":commissionStatus==="NO_APLICA"?"green":"violet"}">👤 ${esc(seller.nombre)}</span>${opType!=="NOTA_CREDITO"?`<span class="pill ${numeric(o.saldo)>0?'amber':'green'}">${numeric(o.saldo)>0?`Saldo ${money(o.saldo)}`:"Pagado"}</span>`:""}${isAnnulled(o.estado)?'<span class="pill red">Anulado</span>':''}</div></div><div class="card-side"><strong class="${opType==="NOTA_CREDITO"?"negative":""}">${opType==="NOTA_CREDITO"?"− ":""}${money(o.total)}</strong><div class="row-actions"><button class="mini-btn" data-operation-detail="${esc(o.operacion_id)}">Ver</button><button class="mini-btn primary" data-operation-print="${esc(o.operacion_id)}">Imprimir</button>${opType==="REMITO"&&active&&canIssueDocuments()?`<button class="mini-btn" data-credit-note="${esc(o.operacion_id)}">Devolver</button>`:""}${opType==="REMITO"&&active&&isAdmin()?`<button class="mini-btn" data-financial-credit="${esc(o.operacion_id)}">Bonificar</button>`:""}${opType==="REMITO"&&active&&commissionStatus==="PENDIENTE"&&isAdmin()?`<button class="mini-btn warning" data-resolve-commission="${esc(o.operacion_id)}">Definir comisión</button>`:""}${isAdmin()&&active?`<button class="mini-btn danger" data-operation-annul="${esc(o.operacion_id)}">Anular</button>`:""}</div></div></article>`}).join("")||'<div class="empty">Todavía no hay comprobantes.</div>';
}
...
function showOperationDetail(id,autoPrint=false){const o=state.gestion.operaciones.find(x=>String(x.operacion_id)===String(id));if(!o)return;const displayNumber=formatOperationNumber(o.numero),type=operationTypeLabel(o.tipo),seller=operationSellerInfo(o),credit=String(o.tipo).toUpperCase()==="NOTA_CREDITO",commissionStatus=operationCommissionStatus(o);const fields=[["Número",`${type} ${displayNumber}`],["Fecha",formatDate(o.fecha)],["Cliente",o.cliente],["Vendedor / comisión",seller.nombre],["Estado",o.estado],["Total",`${credit?"− ":""}${money(o.total)}`]];if(credit){fields.push(["Tipo de crédito",({DEVOLUCION_PRODUCTOS:"Devolución de productos",BONIFICACION_REMITO:"Bonificación comercial",CREDITO_GENERAL:"Crédito general"})[o.credito_tipo]||"Nota de crédito"]);if(o.referencia_numero)fields.push(["Remito original",formatOperationNumber(o.referencia_numero)]);if(o.credito_concepto)fields.push(["Motivo",o.credito_concepto])}else fields.push(["Saldo",money(o.saldo)]);const html=detailHeader(fields)+itemsTable(operationItems(id))+`<p>${esc(o.observaciones||"")}</p>`,actions=[`<button class="btn primary" data-operation-print="${esc(id)}">Imprimir media A4</button>`];if(!credit&&String(o.tipo).toUpperCase()==="REMITO"&&!isAnnulled(o.estado)&&canIssueDocuments())actions.push(`<button class="btn secondary" data-credit-note="${esc(id)}">Devolver productos</button>`);if(!credit&&String(o.tipo).toUpperCase()==="REMITO"&&!isAnnulled(o.estado)&&isAdmin())actions.push(`<button class="btn secondary" data-financial-credit="${esc(id)}">Bonificar</button>`);if(!credit&&commissionStatus==="PENDIENTE"&&isAdmin())actions.push(`<button class="btn secondary" data-resolve-commission="${esc(id)}">Definir comisión</button>`);openDetail(`${type} ${displayNumber}`,html,actions.join(""));if(autoPrint)setTimeout(()=>printOperation(id),250)}
