...
function requestSaleImport(saleId){const sale=saleById(saleId);if(!sale)return toast("La venta ya no está disponible.","error");const used=saleSourceOperations(sale);if(!used.length)return openOperation(null,true,sale);state.pendingSourceReuse={type:"VENTA",id:String(sale.venta_id)};$("#orderReuseEyebrow").textContent="VENTA YA UTILIZADA";$("#orderReuseTitle").textContent=`La venta ${sale.venta_id} ya fue utilizada.`;$("#orderReuseDetail").textContent=`Comprobante${used.length===1?"":"s"} generado${used.length===1?"":"s"}: ${used.map(orderUsedLabel).join(", ")}.`;$("#orderReuseDialog").showModal()}
