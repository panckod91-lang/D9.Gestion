...
function renderOperations() {
  ...
  $("#operationsList").className="card-list"; $("#operationsList").innerHTML=rows.map(o=>`
    <article class="data-card"><div><h3>${esc(operationTypeLabel(o.tipo))} ${esc(formatOperationNumber(o.numero||""))}</h3><p>${esc(o.cliente||"")} · ${formatDate(o.fecha)}</p><div class="meta">${o.referencia_numero?`<span class="pill amber">Sobre ${esc(formatOperationNumber(o.referencia_numero))}</span>`:(o.origen_pedido_id?`<span class="pill">Pedido ${esc(o.origen_pedido_id)}</span>`:'<span class="pill">Carga manual</span>')}<span class="pill violet">👤 ${esc(operationSellerInfo(o).nombre)}</span>${String(o.tipo).toUpperCase()!=="NOTA_CREDITO"?`<span class="pill ${numeric(o.saldo)>0?'amber':'green'}">${numeric(o.saldo)>0?`Saldo ${money(o.saldo)}`:"Pagado"}</span>`:""}${isAnnulled(o.estado)?'<span class="pill red">Anulado</span>':''}</div></div><div class="card-side"><strong class="${String(o.tipo).toUpperCase()==="NOTA_CREDITO"?"negative":""}">${String(o.tipo).toUpperCase()==="NOTA_CREDITO"?"− ":""}${money(o.total)}</strong><div class="row-actions"><button class="mini-btn" data-operation-detail="${esc(o.operacion_id)}">Ver</button><button class="mini-btn primary" data-operation-print="${esc(o.operacion_id)}">Imprimir</button>${String(o.tipo).toUpperCase()==="REMITO"&&!isAnnulled(o.estado)&&canIssueDocuments()?`<button class="mini-btn" data-credit-note="${esc(o.operacion_id)}">Hacer NC</button>`:""}${isAdmin()&&!isAnnulled(o.estado)?`<button class="mini-btn danger" data-operation-annul="${esc(o.operacion_id)}">Anular</button>`:""}</div></div></article>`).join("")||'<div class="empty">Todavía no hay comprobantes.</div>';
}
...
function hydrateSellerSelectors(){
  ...
  if(commission){commission.innerHTML=sellerOptions(commissionCurrent,"Todos los vendedores");if(commissionCurrent)commission.value=commissionCurrent}
}
...
function openCommissionClosure(id){const row=(state.gestion.comisiones_cierres||[]).find(item=>String(item.cierre_id)===String(id));if(!row)return;$("#reportCommissionFrom").value=String(row.desde).slice(0,10);$("#reportCommissionTo").value=String(row.hasta).slice(0,10);$("#reportCommissionSeller").value="";renderCommissionReport();$("#commissionReportStatus").scrollIntoView({behavior:"smooth",block:"center"})}
...
function renderReports(){renderSalesReport();renderCommissionReport()}
...
function openOperation(order=null) {
  ...
  state.currentOrder=order;state.currentCreditOperation=null;state.operationPriceList="lista_1"; state.draftItems=(order?.items||[]).map(i=>({id_producto:i.id_producto||i.id, nombre:i.nombre||i.detalle, cantidad:numeric(i.cantidad||i.total), precio:numeric(i.precio)}));
...
async function saveOperation(event) {
  ...
  if(state.currentCreditOperation&&items.some(item=>numeric(item.cantidad)>numeric(item.maximo)+.0001))return toast("Una cantidad supera lo disponible en el remito.","error");const total=operationTotal(),payments=state.currentCreditOperation?[]:readPayments("op"),paid=payments.reduce((s,p)=>s+Number(p.importe),0);if(paid>total+.01)return toast("El pago inicial no puede superar el total.","error");if(!state.currentCreditOperation&&$("#opPaymentMethod").value!=="CUENTA_CORRIENTE"&&!payments.length)return toast("Ingresá el importe pagado.","error");
  const selectedSeller=sellerById($("#opSeller").value)||(state.currentCreditOperation?operationSellerInfo(state.currentCreditOperation):null),payload={tipo:$("#opType").value,fecha:$("#opDate").value,cliente_id:cliente.id,cliente:cliente.nombre,vendedor_id:selectedSeller?.id||"",vendedor:selectedSeller?.nombre||"",origen_pedido_id:$("#opSourceOrder").value,referencia_operacion_id:$("#opReferenceOperation").value,descuento_pct:Number($("#opDiscount").value)||0,observaciones:$("#opNotes").value.trim(),items,pagos_iniciales:payments};
...
function showOperationDetail(id,autoPrint=false){const o=state.gestion.operaciones.find(x=>String(x.operacion_id)===String(id));if(!o)return;const displayNumber=formatOperationNumber(o.numero),type=operationTypeLabel(o.tipo),seller=operationSellerInfo(o),credit=String(o.tipo).toUpperCase()==="NOTA_CREDITO";const fields=[["Número",`${type} ${displayNumber}`],["Fecha",formatDate(o.fecha)],["Cliente",o.cliente],["Vendedor",seller.nombre],["Estado",o.estado],["Total",`${credit?"− ":""}${money(o.total)}`]];if(credit)fields.push(["Remito original",formatOperationNumber(o.referencia_numero||"")]);else fields.push(["Saldo",money(o.saldo)]);const html=detailHeader(fields)+itemsTable(operationItems(id))+`<p>${esc(o.observaciones||"")}</p>`;openDetail(`${type} ${displayNumber}`,html,`<button class="btn primary" data-operation-print="${esc(id)}">Imprimir media A4</button>${!credit&&String(o.tipo).toUpperCase()==="REMITO"&&!isAnnulled(o.estado)&&canIssueDocuments()?`<button class="btn secondary" data-credit-note="${esc(id)}">Crear nota de crédito</button>`:""}`);if(autoPrint)setTimeout(()=>printOperation(id),250)}
...
function bindEvents(){
  ...
  $("#btnNewUser").addEventListener("click",()=>openUserEditor());$("#userForm").addEventListener("submit",saveUser);$("#commissionForm").addEventListener("submit",saveCommission);$("#userRole").addEventListener("change",updateUserRoleFields);$("#userGestionRole").addEventListener("change",updateUserRoleFields);[["#usersSearch","input"],["#usersStatus","change"],["#usersGestionRole","change"]].forEach(([selector,eventName])=>$(selector).addEventListener(eventName,renderUsers));
  ...
  ["#reportSalesFrom","#reportSalesTo","#reportSalesSeller"].forEach(selector=>$(selector).addEventListener("change",renderSalesReport));$("#btnPrintSalesReport").addEventListener("click",printSalesReport);["#reportCommissionFrom","#reportCommissionTo","#reportCommissionSeller"].forEach(selector=>$(selector).addEventListener("change",renderCommissionReport));$("#btnPrintCommissionReport").addEventListener("click",printCommissionReport);$("#btnCloseCommissions").addEventListener("click",closeCommissions);
}
