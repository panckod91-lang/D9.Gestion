...
function renderOperations() {
  const q=normalize($("#operationsSearch").value),type=$("#operationsType").value,status=$("#operationsStatus").value;
  const rows=[...state.gestion.operaciones].sort((a,b)=>String(b.created_at||b.fecha).localeCompare(String(a.created_at||a.fecha))).filter(o=>{
    const annul=isAnnulled(o.estado);if(type&&String(o.tipo).toUpperCase()!==type)return false;if(status==="active"&&annul)return false;if(status==="annulled"&&!annul)return false;
...
function hydrateSellerSelectors(){
  const clientFilter=$("#clientsSeller"),clientCurrent=clientFilter?.value,report=$("#reportSalesSeller"),reportCurrent=report?.value;
  ...
  if(report){report.innerHTML=sellerOptions(reportCurrent,"Todos los vendedores");if(reportCurrent)report.value=reportCurrent}
}
...
function renderClients(){
  ...
  $("#btnNewClient").disabled=!isAdmin();$("#btnAssignSellers").disabled=!isAdmin();
...
function salesReportTotals(rows){const totals={VENTAS:0,EFECTIVO:0,TRANSFERENCIA:0,CHEQUE:0,CUENTA_CORRIENTE:0};rows.forEach(operation=>{totals.VENTAS+=numeric(operation.total);paymentLabelForOperation(operation).parts.forEach(item=>totals[item.method]=(totals[item.method]||0)+item.amount)});return totals}
...
function renderSalesReport(){
  ...
  list.className="report-table-wrap";list.innerHTML=rows.length?`<table class="report-table"><thead><tr><th>Fecha / hora</th><th>Vendedor</th><th>Cliente</th><th>Comprobante</th><th>Pago inicial</th><th>Total</th></tr></thead><tbody>${rows.map(operation=>{const seller=operationSellerInfo(operation);return `<tr><td>${formatDate(operation.fecha)}<small>${operationTime(operation)}</small></td><td>${esc(seller.nombre)}</td><td>${esc(operation.cliente)}</td><td>${esc(operationTypeLabel(operation.tipo))} ${esc(formatOperationNumber(operation.numero))}</td><td>${esc(paymentLabelForOperation(operation).text)}</td><td><strong>${money(operation.total)}</strong></td></tr>`}).join("")}</tbody></table>`:'<div class="empty">No hay ventas para el período.</div>';
}
...
async function deleteClientFromSellerAssignment(id,button){
  if(!isAdmin())return toast("Esta sesión no puede eliminar clientes.","error");
  const client=adminClients().find(item=>String(item.id)===String(id)),message=$("#sellerAssignmentMessage");
  if(!client)return toast("No encontré el cliente que querés eliminar.","error");
  const originalText=button.textContent;button.disabled=true;button.textContent="…";message.classList.add("hidden");
  try{
    const impact=await apiPost("source_client_delete_impact",{cliente_id:String(id)});
    if(impact.blocked){
      ...
      message.textContent=`No se puede eliminar a ${client.nombre}: tiene ${references}. Primero hay que fusionar su historial con la ficha correcta.`;message.className="form-message error";message.classList.remove("hidden");return;
    }
    const historical=impact.pedidos?`\n\nTiene ${impact.pedidos} pedido${impact.pedidos===1?"":"s"} histórico${impact.pedidos===1?"":"s"}. No se borrará${impact.pedidos===1?"":"n"}.`:"";
    if(!confirm(`¿Eliminar definitivamente a ${client.nombre}?${historical}\n\nLas demás asignaciones todavía no guardadas se conservarán.`))return;
    button.textContent="⌛";const result=await apiPost("source_delete_client",{cliente_id:String(id)});
    state.source.clientes_admin=state.source.clientes_admin.filter(item=>String(item.id)!==String(id));state.source.clientes=state.source.clientes.filter(item=>String(item.id)!==String(id));
    delete state.sellerAssignments[String(id)];delete state.sellerAssignmentOriginal[String(id)];delete state.sellerSuggestions[String(id)];
    saveCurrentCache();hydrateClientFilters();hydrateSellerSelectors();renderSellerAssignments();renderClients();renderOrders();setSync("Datos actualizados");
    message.textContent=`${result.message||"Cliente eliminado"} Las demás asignaciones pendientes siguen listas para guardar.`;message.className="form-message success";message.classList.remove("hidden");toast(result.message||"Cliente eliminado");
  }catch(err){message.textContent=err.message;message.className="form-message error";message.classList.remove("hidden")}
  finally{if(button.isConnected){button.disabled=!sourceWritesEnabled();button.textContent=originalText}}
}
...
async function saveClient(event){
  ...
  try{const result=await apiPost("source_save_client",{cliente,nuevo:$("#clientForm").dataset.mode==="new"});upsertBy(state.source.clientes_admin,"id",cliente);state.source.clientes=state.source.clientes_admin.filter(c=>activeValue(c.activo));saveCurrentCache();toast(result.message||"Cliente guardado");$("#clientDialog").close();hydrateClientFilters();renderClients();refreshAfterMutation()}
  catch(err){message.textContent=err.message;message.className="form-message error";message.classList.remove("hidden")}
...
function populateSelectors() {
  ...
  hydrateSellerSelectors();if(!$("#reportSalesFrom").value)$("#reportSalesFrom").value=todayISO();if(!$("#reportSalesTo").value)$("#reportSalesTo").value=todayISO();
}
...
function selectOperationClient(clientId){
  ...
  if(!$("#opSeller").value&&client.vendedor_id)$("#opSeller").value=String(client.vendedor_id);
...
function startOperationClientSearch(){
  ...
  state.clientSearchResults=[];state.clientSearchIndex=0;state.occasionalClientId="";state.operationPriceList="lista_1";$("#opSeller").value="";renderOperationClientResults();setTimeout(()=>$("#opClientSearch").focus(),30);
}
...
async function saveOperation(event) {
  ...
  const selectedSeller=sellerById($("#opSeller").value),payload={tipo:$("#opType").value,fecha:$("#opDate").value,cliente_id:cliente.id,cliente:cliente.nombre,vendedor_id:selectedSeller?.id||"",vendedor:selectedSeller?.nombre||"",origen_pedido_id:$("#opSourceOrder").value,descuento_pct:Number($("#opDiscount").value)||0,observaciones:$("#opNotes").value.trim(),items,pagos_iniciales:payments};
...
function showOperationDetail(id,autoPrint=false){const o=state.gestion.operaciones.find(x=>String(x.operacion_id)===String(id));if(!o)return;const displayNumber=formatOperationNumber(o.numero),type=operationTypeLabel(o.tipo),seller=operationSellerInfo(o);const html=detailHeader([["Número",`${type} ${displayNumber}`],["Fecha",formatDate(o.fecha)],["Cliente",o.cliente],["Vendedor",seller.nombre],["Estado",o.estado],["Total",money(o.total)],["Saldo",money(o.saldo)]])+itemsTable(operationItems(id))+`<p>${esc(o.observaciones||"")}</p>`;openDetail(`${type} ${displayNumber}`,html,`<button class="btn primary" data-operation-print="${esc(id)}">Imprimir media A4</button>`);if(autoPrint)setTimeout(()=>printOperation(id),250)}
...
function canonicalOperationNumber(value,type){const raw=String(value||""),match=raw.match(/(\d+)\s*$/),prefix=({REMITO:"R",PROFORMA:"FPF",NOTA_VENTA:"NDV"})[String(type||"").toUpperCase()];return prefix&&match?`${prefix}-${match[1].padStart(8,"0").slice(-8)}`:raw}
function operationTypeLabel(value){return ({REMITO:"Remito",PROFORMA:"Factura pro forma",NOTA_VENTA:"Nota de venta"})[String(value||"").toUpperCase()]||String(value||"Comprobante").replace(/_/g," ")}
...
function bindEvents(){
  ...
  $("#btnMore").addEventListener("click",()=>$("#moreDialog").showModal());$("#btnNewProduct").addEventListener("click",()=>openProductEditor());$("#btnBulkPrices").addEventListener("click",openBulkPrices);$("#productForm").addEventListener("submit",saveProduct);$("#bulkPriceForm").addEventListener("submit",applyBulkPrices);$("#btnRefreshBulkPreview").addEventListener("click",calculateBulkPreview);$("#mastersPriceList").addEventListener("change",renderMasters);$("#btnNewClient").addEventListener("click",()=>openClientEditor());$("#clientForm").addEventListener("submit",saveClient);$("#btnDeleteClient").addEventListener("click",deleteClient);$("#btnAssignSellers").addEventListener("click",openSellerAssignment);$("#sellerAssignmentSearch").addEventListener("input",renderSellerAssignments);$("#sellerAssignmentMissing").addEventListener("change",renderSellerAssignments);$("#btnApplySellerSuggestions").addEventListener("click",applySellerSuggestions);$("#btnSaveSellerAssignments").addEventListener("click",saveSellerAssignments);$("#sellerAssignmentList").addEventListener("change",event=>{const select=event.target.closest("[data-seller-assignment]");if(select){state.sellerAssignments[select.dataset.sellerAssignment]=select.value;renderSellerAssignments()}});$("#sellerAssignmentList").addEventListener("click",event=>{const button=event.target.closest("[data-delete-assignment-client]");if(button)deleteClientFromSellerAssignment(button.dataset.deleteAssignmentClient,button)});$$("#clientFiscalDetails input, #clientFiscalDetails select").forEach(input=>input.addEventListener("input",updateClientFiscalStatus));$("#btnNewOffer").addEventListener("click",()=>openOfferEditor());$("#offerForm").addEventListener("submit",saveOffer);$("#offerProduct").addEventListener("input",updateOfferProductInfo);$("#btnDeleteOffer").addEventListener("click",deleteOffer);$("#offersSearch").addEventListener("input",renderOffers);$("#offersStatus").addEventListener("change",renderOffers);
